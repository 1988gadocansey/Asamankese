	...
	public function sendBulkSms($arrayphone,$msg){
        ...
        public function sendSMS1($phone,$message){
            ...
            $fields_string = ""; 
                   ...
                   $info="Message was successfully sent"; 
                   $date=time();
                    $insertor=$this->connect->Prepare("insert into tbl_sms set sent_by='$_SESSION[ID]', number='$phone',type='$type',name='$applicant',message='$message',dates='$date',status='Delivered',year='$year'");
                    $this->connect->Execute($insertor) ;
	
                  return $info;
                    }else{ 
                        $date=time();
                        $insertor=$this->connect->Prepare("insert into tbl_sms set sent_by='$_SESSION[ID]',number='$phone',type='$type',name='$applicant' ,message='$message',dates='$date',status='Not Delivered',year='$year'");
                        $this->connect->Execute($insertor) ;
