...
<body>
  ...
  while($row3 = $result3->FetchRow()){
        ...
        array_push($studentSArray, $row3[student]);
   ...
   $stud=$studentSArray;
    $query = $sql->Prepare("Select *,tbl_class_members.class as stage from tbl_student,tbl_classes,tbl_class_members where tbl_student.INDEXNO='$stud[$i]'  and tbl_class_members.student=tbl_student.indexno and tbl_classes.name=tbl_student.class and tbl_student.status='In school' and tbl_class_members.year='$thisyear' and tbl_class_members.term='$thisterm' ");
   // print_r($query);
		...
		$stmt2=$sql->Prepare("select * from tbl_assesments,tbl_courses,tbl_subjects where tbl_assesments.stuId='".$help->getID($stud[$i])."' and tbl_assesments.year='$yearData' and tbl_assesments.term='$termData' and tbl_assesments.courseId=tbl_courses.id and tbl_subjects.name=tbl_courses.name and tbl_subjects.type='elective' order by tbl_subjects.type ASC,tbl_subjects.name ASC");
          ...
          <tr>
            <td height="43" nowrap='nowrap' ><div align="left"><?php echo $r[name]; ?></div></td>
              ...
              <td ><div align="center"><?php echo $r[exam];?></div></td>
              <td ><div align="center"><?php echo $r[total]; $ttotale+=$r[total]; if($r['total']>0){ $cout=$cout+100;}?></div></td>
              <td width="81" >
                <div align="center">
                  <?php 
                   $stmt=$sql->Prepare("select grade,valu,comment from tbl_gradedefinition where   lower <='$r[total]'  and upper >= '$r[total]'") ;
            ...
            <td  ><?php echo $ttotale."/".$cout; ?></td>
            <td  ><div align="right">Average Score : </div></td>
            <td > <?php echo number_format($ttotale/$ttt, 2, '.', ',')."%"; ?> </td>
